...
void RoomHandler::CreateRoom(int roomId, int hostId)
{
    std::lock_guard<std::mutex> lock(roomsMutex);
    if (m_rooms.find(roomId) == m_rooms.end())
    {       
        std::unique_ptr<Room> newRoom = std::make_unique<Room>(roomId, hostId);
        newRoom->AddUser(hostId);
...
std::string RoomHandler::GetRoomById(int roomId)
{
    ...
    if (it != m_rooms.end())
    {
        ...
        std::string roomDetails = "Room ID: " + std::to_string(roomId) + ", Users: " + std::to_string(room->GetUserCount()) + "\n";
        return roomDetails;
    }
